...
set.seed(1234)
jobRunScript(workingDir = getwd(), path = "NFT_PBPK2/ExtrapolatedV4_ratEHR.R", importEnv = T, exportEnv = "job_421")
set.seed(4234)
jobRunScript(workingDir = getwd(), path = "NFT_PBPK2/ExtrapolatedV4_ratEHR.R", importEnv = T, exportEnv = "job_521")
set.seed(5234)
jobRunScript(workingDir = getwd(), path = "NFT_PBPK2/ExtrapolatedV4_ratEHR.R", importEnv = T, exportEnv = "job_621")
set.seed(6234)
jobRunScript(workingDir = getwd(), path = "NFT_PBPK2/ExtrapolatedV4_ratEHR.R", importEnv = T, exportEnv = "job_721")
...
sims <- mcmc_array(list(job_421$out,job_521$out,job_621$out,job_721$out))  #job_3$out, 
#sims <- mcmc_array(list(job_1$out,job_3$out))  #,job_4$out
endparms = length(names(job_121$out))-3
parms_name_1 <- paste((names(job_121$out)[2:endparms]),sep = ",")

# 
Files = list.files(path = paste0(getwd()),pattern = "^ExtrapolatedV4_ratEHR.*\\.out$",full.names=TRUE)
...
str(dataf)
str(monitor_file)
write.csv(dataf[ ,c("mean","sd","X2.5.","X97.5.","Rhat")], file = paste0(getwd(),"/",Modeling_dir, "/", "Posterior_chains","/", "ExtrapolatedV4ratEHR.csv"))

X <- sims[j,,] %>% matrix(nrow = length(j)*4)

parms_name_2 <- paste((names(job_121$out)),sep = ",")

parms_name_2 <- paste((names(myfiles[[1]])),sep = ",")

filenames <- list.files(path = paste0(getwd(), "/"),pattern = "^ExtrapolatedV4_ratEHR.*\\.out$")
file.copy(from = paste0(getwd(), "/", filenames),
          to = paste0(getwd(), "/",Modeling_dir, "/", "Posterior_chains","/","/", filenames))

colnames(X) = parms_name_2
head(X)
write.table(X, file = paste0(getwd(),"/",Modeling_dir, "/", "Posterior_chains","/","_rat_","ExtrapolatedV4EHR.out"), row.names = F, sep = "\t")

posteriordisfile <- read.delim(paste0(getwd(),"/",Modeling_dir, "/", "Posterior_chains","/","_rat_","ExtrapolatedV4EHR.out"), header = TRUE, sep = "")
...
model = "ExtrapolatedV4_ratEHR.model.R"  # model with EHR and fecal excretion
...
write.table(Simulationfile, file = paste0(getwd(),"/",Modeling_dir, "/", "Posterior_chains","/",exp_route,Species,modelingversion, "ExtrapolatedV4EHR.out"), row.names = F, sep = "\t")

#read.delim(Simulationfile, file = paste0(getwd(),"/",Modeling_dir, "/", "Posterior_chains","/",Species,modelingversion, "ExtrapolatedV4EHR.out"), row.names = F, sep = "\t")
