...
input <- "zeroEHRAgedynamic_montecarlo.in.R"

output <- "zeroEHRMonteHumanPBPK.out"
...
Conditions = c("1.NormalEHR","2_NoEHR")   # as we have 3 variables
Simulationfile$dose_uM = as.numeric(rep(paste0(rep(Final_Rdose1)),each = length(Times)))
Simulationfile$Conditions = ((rep(paste0(rep(Conditions)),each = length(Times)*length(Final_Rdose1))))
Simulationfile$Times <- Times
Simulationfile$variables = (rep(paste0(rep(proteins)),each = length(Times)*length(Final_Rdose1)*length(Conditions)))

colnames(Simulationfile) <- c("median", "LCL", "UCL","mean","dose_uM","Conditions","Times","variables")

#write.table(Simulationfile, file = paste0(getwd(),"/",Modeling_dir, "/", "HumanMontecarloSimulation.out"), row.names = F, sep = "\t")

write.table(Simulationfile, file = paste0(getwd(),"/",Modeling_dir, "/", "Posterior_chains","/", "EHRzeroHumanMontecarloSimulation.out"), row.names = F, sep = "\t")

# length(((rep(paste0(rep(Conditions)),each = length(Times)*biomarker*length(Final_Rdose1)))))
# unique(Simulationfile$dose_uM)

fil3e = Simulationfile %>% filter(.,variables == "cliver") %>% filter(dose_uM == 100) %>% filter(Conditions == "2_NoEHR")
...
plot_function = function (variable1, condition1) {
  Simulation = Simulationfile %>% filter(.,variables == variable1) %>% filter(Conditions == condition1)
...
plot_function(variable1 = "cplasma", condition1 = c("1.NormalEHR","2_NoEHR"))

plot_function(variable1 = "P_excreted", condition1 = c("1.NormalEHR","2_NoEHR"))
