...
#Model version 1: generic model with only GFR based elimination and liver based metabolism in rabbits. 
...
Outputs = {cgut, cliver, cfat,ckidney,cfilterate, crestbody, cplasma,P_excreted,Massbalance,Filteration};	

Inputs = {oraldose1,IVdose1}; 	
...
IVdosing = 0;
oraldose = 0;
Kbp = 0.76;  #0.76;  #0.76;  #blood to plasma partioning
kfeces = 0;
...
Initialize {
  ...
  Qfilterate= 0.631; 				                #Glomerulation filteration rate L/hr/BW (2.5 kg standard body weight)(rabbit paper)  https://doi.org/10.1258/la.2012.011065
...
Dynamics {
  ...
  dt(Akidney) = Qkidney*cplasma*(fu/Kbp) - Qkidney*ckidney*((fu/Kbp)/Kkidney_plasma) -Filteration;
  
  dt(Afilterate) = Filteration - Qfilterate*cfilterate;  # ((Tm*(pow(cfilterate, nt)))/(pow(Kt,nt)+ pow(cfilterate,nt))); #(Tm*cfilterate)/(Kt+cfilterate); 
