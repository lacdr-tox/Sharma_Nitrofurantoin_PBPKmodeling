...
Level { # top   priors on parameters for all chemicals
  ...
  # Distrib(Kgut_plasma, TruncNormal,0.5, 0.1, 0.01, 10);                # exact mean
  # Distrib(Kliver_plasma, TruncNormal, 4.8, 2, 0.01, 10);           # exact mean
  # Distrib(Kkidney_plasma, TruncNormal,  4.347, 2,  1e-2, 20);
  # Distrib(Kfat_plasma , TruncNormal,  0.77,  3,  1e-2, 2);             # exact mean
  Distrib(Krestbody_plasma, TruncNormal,  2, 1,  1e-4, 1);

 #Distrib(kgutabs, TruncNormal, 2,  3,  1e-2, 10);
 #Distrib(fu, TruncNormal, 0.4149, 0.1,  0.25, 0.8);
  Distrib(VmaxC, TruncNormal, 0.043, 0.05,  0.0001, 200);
  Distrib(Km, TruncNormal, 500, 100,  1e-4, 1000);
  #Distrib(kdegliver, TruncNormal, 10, 5,  1e-4, 800);
  Distrib(QurineC, TruncNormal, 10, 5,  1, 300);
  
  Distrib(Tmc , TruncNormal,  20, 5,  1e-6, 500);             # exact mean
  Distrib(Kt , TruncNormal,  0.1, 0.01,  0.0007, 1);             # exact mean
