...
set.seed(1234)
jobRunScript(workingDir = getwd(), path = "NFT_PBPK2/ExtrapolatedRatehrRabbitRenal_mixed.R", importEnv = T, exportEnv = "job_121")
set.seed(4234)
jobRunScript(workingDir = getwd(), path = "NFT_PBPK2/ExtrapolatedRatehrRabbitRenal_mixed.R", importEnv = T, exportEnv = "job_221")
set.seed(5234)
jobRunScript(workingDir = getwd(), path = "NFT_PBPK2/ExtrapolatedRatehrRabbitRenal_mixed.R", importEnv = T, exportEnv = "job_321")
set.seed(6234)
jobRunScript(workingDir = getwd(), path = "NFT_PBPK2/ExtrapolatedRatehrRabbitRenal_mixed.R", importEnv = T, exportEnv = "job_421")
...
sims <- mcmc_array(list(job_121$out,job_221$out,job_321$out,job_421$out))  #job_3$out, 
#sims <- mcmc_array(list(job_421$out,job_221$out,job_321$out))  #job_3$out,
#sims <- mcmc_array(list(job_421$out))  #,job_4$out
endparms = length(names(job_121$out))-3
parms_name_1 <- paste((names(job_121$out)[2:endparms]),sep = ",")

# ExtrapolatedRatehrRabbitRenal_mixed2_9458
# Files = list.files(path = paste0(getwd()),pattern = "^ExtrapolatedRatehrRabbitRenal_mixed2.*\\.out$",full.names=TRUE)
...
str(dataf)
str(monitor_file)
write.csv(dataf[ ,c("mean","sd","X2.5.","X97.5.","Rhat")], file = paste0(getwd(),"/",Modeling_dir,"/Posterior_chains", "/ExtrapolatedRatehrRabbitRenal_mixed.csv"))
...
filenames <- list.files(path = paste0(getwd(), "/"),pattern = "^ExtrapolatedRatehrRabbitRenal_mixed2.*\\.out$")
file.copy(from = paste0(getwd(), "/", filenames),
          to = paste0(getwd(), "/",Modeling_dir,"/Posterior_chains/", filenames))

colnames(X) = parms_name_2
head(X)
write.table(X, file = paste0(getwd(),"/",Modeling_dir, "/", Modeling_dir,"ExtrapolatedRatehrRabbitRenal_mixed.out"), row.names = F, sep = "\t")
