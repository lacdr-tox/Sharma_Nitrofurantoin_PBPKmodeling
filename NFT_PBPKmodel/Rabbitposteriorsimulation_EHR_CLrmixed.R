#Forward simulation of the extrapolation from rats to rabbits using model V4_ehr extrapolated using posterior paramters distribution 
# obtained from both the data inputs
getwd()
setwd("~/MCSim/mod")
library(ggmcmc)
library(tidyverse)
...
#Read the posterior distribution file estimated from the rat EHR mechanism
Species = "rabbits"
...
level = ifelse(Species == "rabbits",2,"NA") #here no role of level as this is non heirarchical methods 
...
randomdata = sample_n(posteriordisfile, 300)
...
#Here you changed the name of the model to run the rat model
...
for (i in 1:length(Final_Rdose1)){
  ...
  write(simulationprint(dose = Final_Rdose1[i],biomarker1 = proteins[1],biomarker2 = proteins[2],biomarker3 = proteins[3],
                        ...
                        biomarker18 = proteins[18],biomarker19 = proteins[19],biomarker20 = proteins[20],biomarker21 = proteins[21],
                        Times), f)
...
# it is okay to use the model below as the structure of the model is the same, here it the rabbit model with EHR mechanism
# This is the posterior simulation using the mixed optimized paramters from both rat ehr and rabbit renal clearance mechanism. 

model <-  "Extrapolated_ratEHR_rabbit.model.R"
...
Expdata = myfiles 
Species = "Rabbit"
myfiles %>% filter(., Variable =="cplasma" & exposure_route == "oraldose")
...
# Simulationfile =read.delim(paste0(getwd(),"/",Modeling_dir, "/", Modeling_dir,exp_route,Species,modelingversion, "Simulation.out"), header = TRUE, sep = "")
# head(Simulationfile)
