...
Outputs = {cgut, cliver, cfat,ckidney,ctubules, crestbody, cplasma,P_excreted,Massbalance,Filteration,Reabsorption,Secretion,Total_renal_Cl,GFR_contribution,Secr_contribution,Reabs_contribution,Re_GFR_contribution,B_excreted};	
...
Initialize {
  ...
  Tr = krc*BW;  #mg/L/kg to whole body weight (mcg/ml to mg/l is equivalent)
  Vehr = Vehrc*BW;
  kbileS = kbile*BW;
  kgutabsS = kgutabs*BW;
  # kst = kstc*BW;
...
Dynamics {
  ...
  dt(Afilterate) = Filteration - Qfilterate*ctubules -Reabsorption + Secretion;  # ((Tm*(pow(ctubules, nt)))/(pow(Kt,nt)+ pow(ctubules,nt))); #(Tm*ctubules)/(Kt+ctubules); 
  
  dt(Adelay)  = Qfilterate*ctubules  -Qurine *Adelay;
