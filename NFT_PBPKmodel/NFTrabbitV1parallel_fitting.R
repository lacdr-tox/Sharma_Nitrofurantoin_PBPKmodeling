...
set.seed(1234)
jobRunScript(workingDir = getwd(), path = "NFT_PBPK2/NFTrabbitV1.R", importEnv = T, exportEnv = "job_1")
set.seed(4234)
jobRunScript(workingDir = getwd(), path = "NFT_PBPK2/NFTrabbitV1.R", importEnv = T, exportEnv = "job_2")
set.seed(5234)
jobRunScript(workingDir = getwd(), path = "NFT_PBPK2/NFTrabbitV1.R", importEnv = T, exportEnv = "job_3")
set.seed(6234)
jobRunScript(workingDir = getwd(), path = "NFT_PBPK2/NFTrabbitV1.R", importEnv = T, exportEnv = "job_4")
...
sims <- mcmc_array(list(job_1$out,job_2$out,job_3$out, job_4$out))
#sims <- mcmc_array(list(job_5$out,job_6$out,job_7$out))
endparms = length(names(job_1$out))-3
parms_name_1 <- paste((names(job_1$out)[2:endparms]),sep = ",")

# 
#files = list.files(path = paste0(getwd()),pattern = "^NFTrabbit_V1.*\\.out$",full.names=TRUE)
...
filenames <- list.files(path = paste0(getwd(), "/"),pattern = "NFTrabbit_V1.*\\.out$")
...
proteins = c("cgut", "cliver", "cfat","ckidney","cfilterate", "crestbody", "cplasma","P_excreted","Massbalance","Filteration","Agutlumen","Adelay","Aurine","","") 
...
simulationprint = function (dose,biomarker1,biomarker2,biomarker3, biomarker4, biomarker5, biomarker6, biomarker7, biomarker8,biomarker9, biomarker10,biomarker11,biomarker12,biomarker13,biomarker14,biomarker15,Times){
...
proteins = c("Massbalance","cplasma","P_excreted", "Filteration","cgut", "cliver", "cfat","ckidney","cfilterate", "crestbody","Agutlumen","Adelay","Aurine")
...
plot_function3(variable1 = proteins[1:13],mainplot = Title1[5] ,labplot = "NFT conc.(mg/L)")
