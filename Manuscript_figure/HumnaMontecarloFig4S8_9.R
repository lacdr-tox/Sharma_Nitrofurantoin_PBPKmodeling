  ...
  mutate(modified_name = if_else(variables == "cplasma", "Plasma conc. (mg/L)", 
                                 if_else(variables == "P_excreted", "Dose excreted in urine (%)", 
                                         if_else(variables == "B_excreted", "Dose excreted in bile (%)",
                                                 if_else(variables == "cliver", "Liver",
...
modified_files$modified_name = factor(modified_files$modified_name, levels = c("Plasma conc. (mg/L)","Dose excreted in urine (%)","Dose excreted in bile (%)",
                                                                               ...
                                                                               "Reabs_contribution","Re_GFR_contribution","cgut", "Liver", "Kidney","Tubules","Fat", "Rest of the body", 
...
plot_function1 = function (variable, mainplot) {
  ...
  ggplot() +
    ...
    labs(x = "Time [h]", y = "Conc.", sec.x="Exposure (mg)",
         title = mainplot, size = 12) +
    scale_y_continuous(trans = 'log10')+
    scale_x_continuous(breaks = c(2,6,12,18,24)) +
...
tiff(paste0(directory_i,"/", "Figure6",".png"), units="in",width=7, height=6, res=300, compression = 'lzw') 
print(P1)
dev.off()
...
plot_function1 = function (variable, mainplot) {
  ...
  ggplot() +
    ...
    facet_grid(modified_name~dose_uM, scales = "free") +
    #facet_grid(modified_name~dose_uM, labeller = label_both(cplasma = "Plasma", P_excreted = "Excreted"))+
    labs(x = "Time [h]", y = " ", sec.x="Exposure (mg)",
         title = mainplot, size = 12) +
    #scale_y_continuous(trans = 'log10')+
    scale_x_continuous(breaks = c(2,6,12,18,24)) +
    scale_color_manual(name = "type",
...
P1 = plot_function1(variable = c("B_excreted", "P_excreted"),mainplot ="")



tiff(paste0(directory_i,"/", "FigS8",".png"), units="in",width=7, height=8, res=300, compression = 'lzw') 
print(P1)
dev.off()
...
plot_function1 = function (variable, mainplot) {
  ...
  ggplot() +
    ...
    facet_grid(modified_name~dose_uM, scales = "free") +
    #facet_grid(modified_name~dose_uM, labeller = label_both(cplasma = "Plasma", P_excreted = "Excreted"))+
    labs(x = "Time [h]", y = "conc.(mg/L)", sec.x="Exposure (mg)",
         title = mainplot, size = 12) +
    #scale_y_continuous(trans = 'log10')+
    scale_x_continuous(breaks = c(2,6,12,18,24)) +
    scale_color_manual(name = "type",
...
P2 = plot_function1(variable = c("cliver", "ckidney", "cfilterate", "cfat", "crestbody"),mainplot ="")


tiff(paste0(directory_i,"/", "FigS9",".png"), units="in",width=7, height=10, res=300, compression = 'lzw') 
print(P2)
dev.off()
